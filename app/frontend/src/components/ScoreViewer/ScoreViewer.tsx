 ...
 *
 * Copyright (c) 2019-2022 - Musmatic authors
 */
...
import { getElementWidth, getElementHeight } from '../../utils';

function ScoreViewer() {
  const [svgWidth, setSvgWidth] = useState(100);
  const [svgContainerWidth, setSvgContainerWidth] = useState(100);
  const [svgContainerHeight, setSvgContainerHeight] = useState(100);
  ...
  useEffect(() => {
    ...
    const initialHeight = getElementHeight(ref.current);
    const zoomCtrlHeight = getElementHeight(zoomControls.current);
    console.log('width?', initialWidth);
    console.log('height?', initialHeight);
    setSvgWidth(initialWidth - 100);
    setSvgContainerWidth(initialWidth);
    setSvgContainerHeight(initialHeight - zoomCtrlHeight);
  }, []);
  ...
  useEffect(() => {
    function handleResize() {
      ...
      const updatedHeight = getElementHeight(ref.current);
      const zoomCtrlHeight = getElementHeight(zoomControls.current);
      setSvgWidth(updatedWidth - 100);
      setSvgContainerWidth(updatedWidth);
      setSvgContainerHeight(updatedHeight - zoomCtrlHeight);
    }
  ...
  return (
    <Flex grow={1} ref={ref}>
      <Flex direction="column">
        <Flex grow={1}>
          <div style={{
            overflow: 'scroll', height: svgContainerHeight, width: svgContainerWidth, maxWidth: svgContainerWidth,
          }}
          >
